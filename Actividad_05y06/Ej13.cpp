...
#include <iostream>
#include <queue>
#include "Class_grafos_string_full_13.hpp"

using namespace std;

string huecoBlanco(string str, int index)
{
    string blank = str;
    blank[index] = '_';
    return blank;
...
//Creamos la función que crea el grafo.
Graph construirGrafo(vector<string> words)
{
    Graph g(false);
    ...
    for (unsigned int i = 0; i < words.size(); i++)
    {
        ...
        {
            string bucket = huecoBlanco(words[i], j);
...
int main()
{
    ...
    //Definimos el grafo.
    Graph g(false);
    g = construirGrafo(words);

    g = bfs(g, g.getVertex("fool"));

    cout<<"El coste temporal de la funcion buildGraph es: O(n^3) por tener 3 bucles for anidados."<<endl;
