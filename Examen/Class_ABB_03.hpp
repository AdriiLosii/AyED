...
class ArbolBusquedaBinario
{
    ...
    public:
        ...
        void cuenta_nodos(NodoArbol *nodoActual)
        {
            if (nodoActual != NULL)
            {
                if ((nodoActual->hijoIzq != NULL) or (nodoActual->hijoDer != NULL))
                {
                    numNodos += 1;
                }

                cuenta_nodos(nodoActual->hijoIzq);
                cuenta_nodos(nodoActual->hijoDer);
            }
        }
        ...
        {
            numNodos = 0;
            this->cuenta_nodos(raiz);
            return numNodos;
        }

        void cuenta_hojas(NodoArbol *nodoActual)
        {
            if (nodoActual != NULL)
            {
                if ((nodoActual->hijoIzq == NULL) and (nodoActual->hijoDer == NULL))
                {
                    numHojas += 1;
                }

                cuenta_hojas(nodoActual->hijoIzq);
                cuenta_hojas(nodoActual->hijoDer);
            }
        }
        ...
        {
            numHojas = 0;
            this->cuenta_hojas(raiz);
            return numHojas;
        }

        void obten_profundidad(NodoArbol *nodoActual)
        {
            if (nodoActual != NULL)
            {
                prof += 1;
                obten_profundidad(nodoActual->hijoIzq);
                if (prof > prof_maxima)
                {
                    prof_maxima = prof;
                }
                prof -= 1;
                prof += 1;
                obten_profundidad(nodoActual->hijoDer);
                if (prof > prof_maxima)
                {
                    prof_maxima = prof;
                }
